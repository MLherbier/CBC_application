...
The goal of this analysis was to compute the total static biodiversity loss due to land use caused by wheat production in several countries for the year 2019.

Wheat dataset was retrieved on the FAO website (<https://www.fao.org/faostat/en/#data/QCL>). Both area harvested and production figures were downloaded for all countries in 2019 and are respectively stored in the files area.csv and production.csv.
...
Let's first standardize country headers for all data.
...
It means retaining only impact factors for wheat, the ISO-2 code for countries, and only production quantities data (that we rename production instead of Value).
...
A more basic representation of the data : 
...
Now we will look at the total terrestrial effects of wheat production and no longer restrict the analysis to static impacts. It means including dynamic (land conversion) & static effects, fragmentation, encroachment and climate change impacts in our calculations.

However, we will restrict the analysis to terrestrial land as the calculations for aquatic land may be a bit different and I did not have enough time to properly understand how these work.

Let's compute the sum of the effects of wheat for 2019. Here we also multiply the impacts by the production to get the impacts in every country. Then we add them together to get the total effect in every country. Again the values are rounded for convenience.
